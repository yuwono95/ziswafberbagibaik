    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.input-perolehans.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nomorhp') ? 'is-invalid' : '' }}" type="text" name="nomorhp" id="nomorhp" value="{{ old('nomorhp', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('namabank') ? 'is-invalid' : '' }}" name="namabank_id" id="namabank_id" required>
                    @foreach($namabanks as $id => $entry)
                        <option value="{{ $id }}" {{ old('namabank_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
