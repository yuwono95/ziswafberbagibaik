...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.verifikasiPerolehan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.verifikasi-perolehans.update", [$verifikasiPerolehan->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('verificator') ? 'is-invalid' : '' }}" name="verificator_id" id="verificator_id" required>
                    @foreach($verificators as $id => $entry)
                        <option value="{{ $id }}" {{ (old('verificator_id') ? old('verificator_id') : $verifikasiPerolehan->verificator->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
